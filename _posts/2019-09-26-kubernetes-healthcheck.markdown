...
쿠버네티스는 각 컨테이너의 상태를 주기적으로 체크해서, 문제가 있는 컨테이너를 자동으로 재시작하거나 또는 문제가 있는 컨테이너(Pod를) 서비스에서 제외할 수 있다. 이러한 기능을 Health Check라고 하는데 크게 두 가지 방법이 있다.
...
헬스 체크 방식은 여러가지가 있는데, <u>(1) HTTP 를 이용하는 방식 (2) TCP를 이용하는 방식 (3) 쉘 명령어를 이용하는 방식</u> 3가지가 있다. 
