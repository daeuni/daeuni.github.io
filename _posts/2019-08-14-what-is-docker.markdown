...
<hr>
FROM 
   : 컨테이너의 운영 역할을 할 도커 이미지(운영체제)를 정의한다. 여기서는 우분투 토커 이미지를 지정한다.<br>COPY 
   :  방금 작성한 shell script file(helloworld)을 도커 컨테이너 안의 /usr/local/bin에 복사하라고 정의한다.<br>RUN 
   : 도커 컨테이너 안에서 어떤 명령을 수행하기 위한 것으로 helloworld script에 실행 권한을 부여하기 위해 사용한다. 여기까지가 도커 빌드 과정에서 실행되며 그 결과 새로운 이미지가 만들어진다.<br>CMD 
